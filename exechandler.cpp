...
static void callInstBefore(ExecHandler* execHandler, THREADID threadId, ADDRINT instAddr, UINT32 rtnId)
{
    //cout << "routineCallAnyBefore " << execHandler->routines[rtnId].name << endl;
    execHandler->handleCallInst(instAddr, threadId);
}

static void callInstAfter(ExecHandler* execHandler, THREADID threadId, ADDRINT instAddr, UINT32 rtnId)
{
...
void ExecHandler::handleCallInst(ADDRINT instAddr, THREADID threadId)
{
    Locker locker(&lock, threadId);
